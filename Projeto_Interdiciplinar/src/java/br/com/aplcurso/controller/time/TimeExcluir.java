 ...
 * @author Aluno
 */
@WebServlet(name = "TimeExcluir", urlPatterns = {"/TimeExcluir"})
public class TimeExcluir extends HttpServlet {
protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (dao.excluir(idTime)) {
                mensagem = "Time excluido com Sucesso!";
            } else {
                mensagem = "Problemas ao excluir Time";
            }
            request.setAttribute("mensagem", mensagem);            
            response.sendRedirect("TimeListar");
        } catch (Exception ex) {
            System.out.println("Problemas no Servelet ao excluir Time! Erro: "+ ex.getMessage());
            ex.printStackTrace();
        }
    }
